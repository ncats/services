 ...
 */
exports.getPackageLscSettings = function (manifest) {
    if (!manifest || !manifest.lsc) {
      return null;
    }
    const lsc = manifest.lsc;
    //format package dependencies
    lsc.packageDependencies = exports.getPackageDependencies(lsc);
    return lsc;
  }
 ...
 * @param {String} packagePath - A path to a directory containing a LabShare package.
 * @param {Function} func - A function that accepts a path to a LabShare project
 * @param thisArg The context of `func`
 */
exports.applyToNodeModulesSync = function (packagePath, func, thisArg) {
    assert.ok(_.isString(packagePath), 'applyToNodeModulesSync: `root` must be a non-empty string');
    assert.ok(_.isFunction(func), 'applyToNodeModulesSync: `func` must be a function');

    if (!exports.isPackageSync(packagePath)) {
        throw new Error(`${packagePath} must contain a LabShare package!`);
    }
    ...
    for (const dependencyObj of dependencies) {
        ...
        func.call(thisArg, dependencyPath);
    }
};
