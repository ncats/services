...
class ApiLoader {
    constructor(router, options = {}) {
        ...
        // format: {packageName: [routes], ...}
        this.services = new Proxy({}, {
            set(services, packageName, routes) {
                if (!routes || _.some(routes, _.isEmpty)) {
                    throw new TypeError(`Attempted to assign empty route object to ApiLoader.services from package name "${packageName}". Routes: ${JSON.stringify(routes)}`);
                }

                return services[packageName] = routes;
            }
        });
    ...
    _assignRoute(route, router, packageName) {
        let args = _.flatten([route.path, route.middleware]),
            methods = (Array.isArray(route.httpMethod) ? route.httpMethod : [route.httpMethod]).map(method => (method || '').toLowerCase());

        if (!_.some(methods, method => _.isFunction(router[method]))) {
