...
class SocketIOLoader extends EventEmitter {
    ...
    connect() {
        ...
        _.each(this.options.socket.connections, connection => {
            // Set "rejectUnauthorized" to false to support connections to HTTPS hosts. See:
            // https://github.com/socketio/socket.io-client/issues/982#issuecomment-240967725
            const client = clientio.connect(connection, {rejectUnauthorized: false});
