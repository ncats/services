...
class Services {
  ...
  async start() {
    ...
    const lb4ProxySettings = {
        rest: {
            listenOnStart: false,
            expressSettings: {'x-powered-by': false}
        }
    }
    const lb4ProxyApp = new LoopbackProxyApplication(Object.assign(lb4ProxySettings, this._config));
    this._app.use(mountPoint, lb4ProxyApp.requestHandler)

    this._options.logger.info(`Loading Loopback APIs`)
    const loopbackLoader = new LoopbackLoader(Object.assign(lb4ProxySettings, this._config));
    const lbApis = loopbackLoader.loadAPIs();

    for (const api of lbApis) {
      this._app.use('/', api.app.requestHandler);
      await api.app.boot();
      await api.app.start();
    }
